...
(define-public (deposit (amount uint) (user principal))
  (begin
    ...
    ;; Forward to Bitflow stxBTC pool and receive LP shares back.
    ;; ;;CONFIRM: verify add-to-position function name against Bitflow testnet contract
    ;; ;;         STRP7MYBHSMFH5EGN3HGX6KNQ7QBHVTBPF1669DW via Hiro Explorer
    (let ((lp-received
            (try! (as-contract
              (contract-call?
                'STRP7MYBHSMFH5EGN3HGX6KNQ7QBHVTBPF1669DW
                add-to-position
                amount)))))
...
(define-public (withdraw (amount uint) (user principal))
  (begin
    ...
    (let ((shares (default-to u0 (map-get? user-shares user))))
      ...
      ;; Redeem LP shares from Bitflow. Gross sBTC (principal + yield) returns here.
      ;; ;;CONFIRM: verify reduce-position function name against Bitflow testnet contract
      (let ((gross
              (try! (as-contract
                (contract-call?
                  'STRP7MYBHSMFH5EGN3HGX6KNQ7QBHVTBPF1669DW
