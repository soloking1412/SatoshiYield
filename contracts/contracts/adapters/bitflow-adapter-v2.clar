...
(define-private (try-commit-consensus (new-bps uint))
  (let
    ((r0 (default-to u0 (map-get? oracle-reports u0)))
     ...
     (live1 (> b1 u0))
     (live2 (> b2 u0)))
    ;; TESTNET: 1-of-3 quorum so a single oracle key is sufficient.
    ;; MAINNET: restore the three pair-checks for 2-of-3 consensus.
    (if live0 (ok r0)
      (if live1 (ok r1)
        (if live2 (ok r2)
          err-no-consensus)))))
...
(define-public (deposit (amount uint) (user principal))
  (begin
    ...
    ;; TESTNET STUB: pure accounting, no external protocol call.
    ;; MAINNET: forward sBTC to Bitflow pool via add-to-position.
    (let ((existing (default-to u0 (map-get? user-shares user))))
      ...
      (var-set lp-token-balance (+ (var-get lp-token-balance) amount))
      (ok amount))))
    ...
    (let ((shares (default-to u0 (map-get? user-shares user))))
      ...
      (var-set lp-token-balance
        (if (>= (var-get lp-token-balance) amount)
          (- (var-get lp-token-balance) amount) u0))
      ;; TESTNET STUB: return sBTC 1:1. MAINNET: redeem from Bitflow via reduce-position.
